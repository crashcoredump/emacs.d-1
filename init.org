...
*** ~require-package~
Define ~require-package~ for easier installing of
packages.
...
Install all packages here using ~require-package~.
...
Then use the appropriate ~web-mode~ engine when visiting a particular filetype.
