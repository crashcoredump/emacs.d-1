...
*** Other
Enable pixelwise resizing of frames, so they can be properly aligned
by our window manager.
#+BEGIN_SRC emacs-lisp
...
Define LSP client for Python. This defines a ~lsp-python-enable~
function which we use to hook into ~python-mode~.
#+BEGIN_SRC emacs-lisp
  (require 'lsp-mode)
...
When switching focus to another frame, re-activate the proper virtualenv for that
frame's project.
#+BEGIN_SRC emacs-lisp
...
Configure =frame-workflow= and =frame-purpose= to have dedicated
...
When creating a new Projectile frame, we want the frame's name to be
